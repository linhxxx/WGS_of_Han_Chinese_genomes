...
scoreset* drive_scoring_inference
   (void*			_params,
	...
	char			scoreFileId[20];
	score			maxSubScore, minSubScore, scaleTo, scaleBy;
	...
	if (((control*) _params)->ic.gapIterations > 0)
		suicide ("gap scoring inference is currently blocked;  to unblock, contact author");
#endif
	...
	scaleBy = 1.0; // (in case params->ic.subIterations == 0)
	for (trial=1 ; (!inOrbit)&&(trial<=params->ic.subIterations) ; trial++)
		...
		scaleBy = align_for_sub_scores (scaleTo);
	...
	for (trial=1 ; (!inOrbit)&&(trial<=params->ic.gapIterations) ; trial++)
		...
		scaleBy = align_for_gap_scores (scaleTo);
		rewind_sequence_file (query);
...
static void align_for_stats
   (hitprocessor	hitProc,
	void*			hitProcInfo)
	{
	int hspsAreAdaptive = (params->hspThreshold.t != 'S');
...
static double infer_substitution_scores
   (double	pOpen,
	...
	//            the computational gain from doing so is incosequential, and
