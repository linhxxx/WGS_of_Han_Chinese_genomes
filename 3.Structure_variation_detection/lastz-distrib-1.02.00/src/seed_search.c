...
//#define debugDiag 3910-65463	// if defined, breakdown what happens with
...
//
// (3)	searchLimit is not a hard limit, and it there are many circumstances
//		by which we will report more seeds/hits/HSPs/alignments than the limit.
...
static u64 private_hit_search (void)
	...
	empty:
		...
		for ( ; q<qStop ; q++)
			...
			if ((searchLimit > 0) && (searchToGo <= 0)) return basesHit;
...
static u64 find_table_matches
   (u32		packed2,
	...
	for (pos=pt->last[packed2] ; pos!=noPreviousPos ; pos=pt->prev[pos])
		...
		//if ((searchLimit > 0) && (searchToGo <= 0)) break;
