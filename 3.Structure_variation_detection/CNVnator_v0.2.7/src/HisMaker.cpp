...
void HisMaker::pe(string *files,int n_files)
{
  ...
  while (input != "exit" && input != "quit") {
    ...
    if (parseInput(input,chrom,start,end,option)) {
      ...
      if (option == "view") {
	//generateView(chrom,s,e,useATcorr,useGCcorr);
      } else if (option == "genotype") {
// 	for (int i = 0;i < n_files;i++) {
//           Genotyper gen(this,files[i],bin_size);
//           gen.printGenotype(chrom,s,e,useATcorr,useGCcorr);
//         }
      } else if (forDel || forTdup) {
	int win = l;
	...
	for (int f = 0;f < n_files;f++) {
          cout<<">"<<files[f]<<endl;
	  AliParser *parser = new AliParser(files[f].c_str(),true);
	  ...
	  while (parser->parseRecord()) {
	    ...
	    int over = emin - smax + 1;
	    if (over < 0.8*l || over < 0.8*fl) continue;
	    //if (over < 0.5*l || over < 0.5*fl) continue;
	    ...
	    if (iter == qual_hash.end()) { // First read
	      ...
	      if (other_qual >= 20 || parser->getQuality() >= 20)
		cout<<input<<" "
			 ...
			 int range)
{
  ...
  for (int c = 0;c < n_chroms;c++) {
	...
	if (!mask[b]) level[b] = rd[b];
      ...
      calcLevels(level,mask,n_bins,bin_band,sigma,mean,skipMasked);
